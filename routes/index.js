...
var db = require('../db');
var isPaid = require('../middlewares/check')
...
router.post('/:id(\\d+)/delete', ensureLoggedIn,isPaid, function(req, res, next) {
  db.run('DELETE FROM todos WHERE id = ? AND owner_id = ?', [
    req.params.id,
    req.user.id
  ], function(err) {
...
router.post('/toggle-all', ensureLoggedIn,isPaid, function(req, res, next) {
...
router.post('/clear-completed', ensureLoggedIn,isPaid, function(req, res, next) {
